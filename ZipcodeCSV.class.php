...
class ZipcodeCSV {
	...
	public function convert($csv_path, $save_path = null) {
		...
		while ((list(
			...
			// 複数行に分割記載しているデータ対策
			if ($this->_isSplitAddress($duplicate_flg, $zipcode, $prev_zipcode)) {
				continue;
			}

			// 町域名データ補正
			$community_area = $this->_convertCommunityAreaName($community_area);

			// 重複排除処理
			$unique_key	= $this->_getUniqueKey($zipcode, $pref_nm, $city, $community_area);
			if (! isset($processed[$unique_key])) {
				fputcsv($fp_conv, array($jis_cd, $zipcode, $pref_nm, $city, $community_area));
			}
	 ...
	 * 
	 * @access	private
	 * @param	array	$duplicate_flg	行データ
	 * @param	string	$zipcode		郵便番号
	 * @param	string	$prev_zipcode	1つ前の郵便番号
	 * @return	boolean
	 */
	private function _isSplitAddress($duplicate_flg, $zipcode, $prev_zipcode) {
		if ($duplicate_flg != '1' && $zipcode == $prev_zipcode) {
			return true;
		}
		return false;
	}
